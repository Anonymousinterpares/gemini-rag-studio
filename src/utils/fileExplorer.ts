...
export async function scanDirectoryHandle(directoryHandle: FileSystemDirectoryHandle, currentPath: string = ''): Promise<FileSystemItem[]> {
  const items: FileSystemItem[] = [];
  ...
  for await (const handle of entries) {
    ...
    if (handle.kind === 'file') {
      ...
      const subItems = await scanDirectoryHandle(handle as FileSystemDirectoryHandle, path);
      items.push(...subItems);
    }
  }
  return items;
